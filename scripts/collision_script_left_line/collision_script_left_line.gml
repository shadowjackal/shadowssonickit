function collision_script_left_line(argument0) {
	mask = argument0;
	line = floor(mask*2.5);
	...
	if dead = 0 && (action != 1 && action != 2) && collision_line(x-(acos*mask)+(asin*mask),y+(asin*mask)+(acos*mask),x-(acos*mask)+(asin*line),y+(asin*mask)+(acos*line),obj_box_wall,true,true) return true;
	return false;
}
